...
export class EditPurchaseComponent implements OnInit,OnDestroy {
  ...
  getRowData(){
      ...
      this.appService.postData('purchase/get',data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
        var r:any=res;
        this.rowData = r.record;
        this.ingredients = [];
        if(this.rowData.items.length > 0){
            $.each(this.rowData.items, (index: any, value: any ) => {
              this.ingredients.push({
                id:value.id,
                material_category_id: value.material_category_id,
                raw_material_id:value.material_id,
                unit:value.unit,
                qty:value.qty,
                rate:value.rate,
                total:value.amount
...
addMoreRow(){
 ...
 removeRow(index:number){
  ...
  Update(){
    ...
    this.appService.postData('purchase/update',data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
